...
class DAB extends Game {
  ...
  countQuads: (p: number) => number;
  getLineIndex: (pt: Point) => number;
  getAllQuads: (idx: number) => Quad[];
  ...
  constructor(cnt = 5) {
    ...
    this.canvas.addEventListener("mousemove", (ev: MouseEvent) => this.move(ev));
    this.canvas.addEventListener("touchstart", (ev: TouchEvent) => this.click(ev, null));
    this.countQuads = (p: number) => { return this.quads.filter(q => q.color === this.player[p].color).length; }
    this.getLineIndex = (pt: Point) => { return this.lines.findIndex(e => e.hasPoint(pt) && !e.selected); }
    this.getLine = (idx: number) => { return this.lines.find(e => e.index === idx); }
    this.getAllQuads = (idx: number) => { return this.quads.filter(e => e.hasLines(idx) && !e.setted); }
    this.draw = () => {
  ...
  move(ev: MouseEvent): void {
    ...
    const li = this.getLineIndex(new Point(ev.clientX - (ev.srcElement as HTMLCanvasElement).offsetLeft, ev.clientY - (ev.srcElement as HTMLCanvasElement).offsetTop))
  ...
  click(tev: TouchEvent, mev: MouseEvent): void {
